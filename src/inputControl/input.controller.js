...
export const saveInput = async (req, res) => {
    try {
        ...
        const inputRecord = new Input({
            product: product._id, //D del producto encontrado
            quantityAdded,
            employee: employeeId
        });
        ...
        product.stock += quantityAdded; // Actualiza el stock
        await product.save();
...
export const deleteInput = async (req, res) => {
    try {
        const { id } = req.params;
        const deactivatedInput = await Input.findByIdAndUpdate(id, { estado: false }, { new: true }// devuelve el documento actualizado
        );

        // Verificamos si se encontro y desactivó el input
        if (!deactivatedInput) {
            return res.status(404).json({
...
export const getMovementsInventory = async (req, res) => { // Resumen de entradas y salidas 
  try {
    const product = req.product;
...
export const getTopMovedProducts = async (req, res) => { // Productos más movidos. Esta función obtiene los productos más movidos en base a las entradas y salidas
    try {
      // Agrupa y suma entradas por producto
      ...
      // Agrupa y suma salidas por producto
      ...
      // Fusiona los datos entrada y salida
      const movimientos = {};
      ...
      // Convierte a array y suma total movimientos
